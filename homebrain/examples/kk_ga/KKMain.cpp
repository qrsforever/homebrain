...
static KKDeviceInfo gKKHostInfo;
            ...
            if (mGA.yield(300) < 0) {
                ...
                if (errCount == 30) {
                    mainHandler().sendMessage(
                        mainHandler().obtainMessage(KKMT_HB_GAGENT, HB_GAGENT_CONNECT, 0));
                    break;
                }
            }
        }
...
class KKDeviceCallback : public deviceCallBackHandler {/*{{{*/
...
#endif
            mainHandler().sendMessage(mainHandler().obtainMessage(KKMT_HB_GAGENT, arg1, 0, data));
...
class KKRemoteEventCallback : public RemoteEventCallback {/*{{{*/
public:
    KKRemoteEventCallback(): mFirstFlg(true) { }
    ~KKRemoteEventCallback() {}
...
#endif
            mainHandler().sendMessage(
                mainHandler().obtainMessage(KKMT_HB_GAGENT, HB_GAGENT_SUBDEV_TOPOADD, 0, data));
#ifndef USE_SHARED_PTR
            data->safeUnref();
#endif
        }
...
#endif
        mainHandler().sendMessage(
            mainHandler().obtainMessage(KKMT_HB_UPGRADE, HB_UPGRADE_BURNING, 0, data));
#ifndef USE_SHARED_PTR
        data->safeUnref();
#endif
        return 0;
    ...
    int doUpgradeCheck(std::string text) {
        LOGD("(%s)\n", text.c_str());
        mainHandler().removeMessages(KKMT_HB_GAGENT, HB_GAGENT_UPGRADE_CHECK, 0);
        mainHandler().sendMessage(mainHandler().obtainMessage(
                KKMT_HB_GAGENT, HB_GAGENT_UPGRADE_CHECK, 1 /* force report */));
        return 0;
    }

    int doLogReport(std::string elinkId, std::string key, std::string logId) {
        KKDumpInfo(); /* don't remove this line, just let log file flush */
#ifdef USE_SHARED_PTR
        ...
        switch (msg->what) {
            case MT_NETWORK:
                if (msg->arg2 == NETWORK_CONNECT_SUCCESS) {
                    if (!gKKHostInfo.nEid.empty()) {
                        mainHandler().sendMessage(
                            mainHandler().obtainMessage(
                                KKMT_HB_REGISTER, HB_REGISTER_EVENT_SUCCESS, 0));
                        return false;
                    }
                    mainHandler().sendMessage(
                        mainHandler().obtainMessage(KKMT_HB_REGISTER, HB_REGISTER_EVENT_REQUEST, 0));
                    return false;
                }
                break;
            case KKMT_HB_REGISTER:
                switch (msg->arg1) {
                    case HB_REGISTER_EVENT_REQUEST:
                        do {
                            LOGI("KKMT_HB_REGISTER.HB_REGISTER_EVENT_REQUEST\n");
                            std::string payload;
                            payload.append("{\"productKey\": \"").append(gKKHostInfo.nKey).append("\",");
                            payload.append("\"deviceName\": \"").append(gKKHostInfo.nName).append("\",");
                            payload.append("\"deviceId\": \"").append(gKKHostInfo.nDid).append("\",");
                            payload.append("\"manufacture\": \"").append("letv\"}");
                            payload = UTILS::easyPost(ELINK_CLOUD_HOST, payload.c_str(), payload.size());
                            if (payload.empty()) {
                                mainHandler().sendMessage(
                                    mainHandler().obtainMessage(KKMT_HB_REGISTER, HB_REGISTER_EVENT_FAIL, 0));
                                return false;
                            }
                            LOGD("payload: [%s]\n", payload.c_str());
                            Document doc;
                            ...
                            if (doc.HasParseError() || !doc.HasMember("data")) {
                                ...
                                mainHandler().sendMessage(
                                    mainHandler().obtainMessage(KKMT_HB_REGISTER, HB_REGISTER_EVENT_FAIL, 1));
                                return false;
                            }
                            Value &data = doc["data"];
                            if (!data.HasMember("deviceId") || !data.HasMember("elinkId") ||
                                ...
                                mainHandler().sendMessage(
                                    mainHandler().obtainMessage(KKMT_HB_REGISTER, HB_REGISTER_EVENT_FAIL, 2));
                                return false;
                            }
                            Value &elinkId = data["elinkId"];
                            Value &deviceSecret = data["deviceSecret"];
                            gKKHostInfo.nEid = elinkId.GetString();
                            gKKHostInfo.nSecret = deviceSecret.GetString();
                            mainDB().updateOrInsert(gKKHostInfo);
                            mainHandler().sendMessage(
                                mainHandler().obtainMessage(KKMT_HB_REGISTER, HB_REGISTER_EVENT_SUCCESS, 0));
                        } while (0);
                        break;
                    case HB_REGISTER_EVENT_SUCCESS:
                        LOGI("KKMT_HB_REGISTER.HB_REGISTER_EVENT_SUCCESS\n");
                        mainHandler().mCallback = gStateNormal;
                        GAgent().init(ELINK_EMQ_HOST, gKKHostInfo.nEid,
                            gKKHostInfo.nEid, gKKHostInfo.nSecret, gKKHostInfo.nKey);
                        mainHandler().sendMessage(
                            mainHandler().obtainMessage(KKMT_HB_GAGENT, HB_GAGENT_CONNECT, 0));
                        break;
                    case HB_REGISTER_EVENT_FAIL:
                        LOGW("KKMT_HB_REGISTER.HB_REGISTER_EVENT_FAIL\n");
                        mainHandler().sendMessageDelayed(
                            mainHandler().obtainMessage(KKMT_HB_REGISTER, HB_REGISTER_EVENT_REQUEST, 0), 10000);
                        break;
                }
                break;
            default:
    ...
    bool handleMessage(Message *msg) {
        ...
        switch (msg->what) {
            case KKMT_HB_GAGENT:
                switch(msg->arg1) {
                    case HB_GAGENT_CONNECT:
                        if (GAgent().connect() < 0) {
                            ...
                            mainHandler().sendMessageDelayed(
                                mainHandler().obtainMessage(KKMT_HB_GAGENT, HB_GAGENT_CONNECT, 0), 5000);
                        } else {
                            startGatewayAgentThread();
                            if (!mainHandler().hasMessages(KKMT_HB_GAGENT, HB_GAGENT_UPGRADE_CHECK, 0)) {
                                deviceManager().init();
                                mainHandler().sendMessage(mainHandler().obtainMessage(
                                        KKMT_HB_GAGENT, HB_GAGENT_UPGRADE_CHECK, 1 /* force report */));
                            }
                    ...
                    case HB_GAGENT_UPGRADE_CHECK:
                        ...
                        mainHandler().sendMessageDelayed(mainHandler().obtainMessage(
                                KKMT_HB_GAGENT, HB_GAGENT_UPGRADE_CHECK, 0), UPGRADE_CHECK_INTERVAL);
                        return true;
                ...
                break;
            case KKMT_HB_UPGRADE:
                switch(msg->arg1) {
                    case HB_UPGRADE_HOSTINFOFILE:
                        do {
                            LOGI("KKMT_HB_UPGRADE.HB_UPGRADE_HOSTINFOFILE [%s]\n", gHostInfoFile.c_str());
                            ...
                            if (of.is_open()) {
                                ...
                                std::vector<KKDeviceInfo> devs;
                                mainDB().queryBy(devs, KKDeviceInfo());
                    ...
                    case HB_UPGRADE_BURNING:
                        do {
                            ...
                            LOGI("KKMT_HB_UPGRADE.HB_UPGRADE_BURNING [%s]\n", newVersion.c_str());
                            // KKOTAUpgrade(newVersion);
                        } while(0);
                        break;
                    default:
                        ;
            ...
            case MT_DB:
                if (msg->arg1 == DB_TABLE_UPDATE && msg->arg2 == TABLE_TYPE_DEVICE) {
                    ...
                    mainHandler().sendMessage(
                        mainHandler().obtainMessage(KKMT_HB_UPGRADE, HB_UPGRADE_HOSTINFOFILE, 0));
                }
                break;
            default:
                ;
...
public:
    ...
    void run() {
         ...
         *  Log Setting
         *-----------------------------------------------------------------*/
        std::vector<KKLogInfo> logs;
        mainDB().queryBy(logs, KKLogInfo());
         ...
         *   Device Manager
         *-----------------------------------------------------------------*/
        deviceManager().getHostDeviceId(gKKHostInfo.nDid);
        deviceManager().setCallback(new KKDeviceCallback());

        std::vector<KKDeviceInfo> devs;
        mainDB().queryBy(devs, gKKHostInfo.nDid);
        if (devs.size() == 1)
            gKKHostInfo = devs[0];
        else {
            gKKHostInfo.nName.assign("HomeBrain");
            gKKHostInfo.nKey.assign("pk.33ef1b64b64");
            gKKHostInfo.nType = DEVICE_TYPE_GATEWAY;
            mainDB().updateOrInsert(gKKHostInfo);
        }
         ...
         *   Gateway Agent
         *-----------------------------------------------------------------*/
        GAgent().setRemoteCallback(new KKRemoteEventCallback());
...
{
    printf("###main###[%lu] [%s] [%s]\n", pthread_self(), KKBuildVersion(), KKBuildDatetime());
