...
public:
    ...
    Tv() : m_state(false), m_power(0),m_heartbeat(0), m_name("")
    {
    }
};
...
{
    static int pre_heart = 0;
	static int count=0;
while(1){

	if(pre_heart!=mytv.m_heartbeat){
		pre_heart=mytv.m_heartbeat;
		count=0;
	}
	else
		count++;
	...
	if(count>3)
	{
		...
		std::cout << "TV Power on>>>>>>>>>>>>>>>>>>>>" <<mytv.m_heartbeat<< std::endl;
	}
sleep(1);
}
}
                    ...
                    const int& eCode, const int& sequenceNumber)
{
    try
    {
	    ...
	    mytv.m_heartbeat++;
            ...
            //rep.getValue("state", mytv.m_state);
            //rep.getValue("power", mytv.m_power);
            //rep.getValue("name", mytv.m_name);
	    //rep.getValue("heartbeat", mytv.m_heartbeat);
	    
            //std::cout << "\tstate: " << mytv.m_state << std::endl;
            //std::cout << "\tpower: " << mytv.m_power << std::endl;
            //std::cout << "\tname: " << mytv.m_name << std::endl;
	    //std::cout << "\theartbeat: " << mytv.m_heartbeat << std::endl;
/*
            if(observe_count() == 50)
            {
        ...
        const OCRepresentation& rep, const int eCode)
{
    try
    {
                ...
                || eCode == OC_STACK_RESOURCE_CHANGED)
        {
            ...
            std::cout << "Posting Tv representation..."<<std::endl;

            mytv.m_state = true;
            mytv.m_power = 55;

            rep2.setValue("state", mytv.m_state);
            rep2.setValue("power", mytv.m_power);
...
void postTvRepresentation(std::shared_ptr<OCResource> resource)
{
    if(resource)
    {
        ...
        rep.setValue("state", mytv.m_state);
        rep.setValue("power", mytv.m_power);
...
void onPut(const HeaderOptions& /*headerOptions*/, const OCRepresentation& rep, const int eCode)
{
    try
    {
        if (eCode == OC_STACK_OK || eCode == OC_STACK_RESOURCE_CHANGED)
        {
            ...
            std::cout << "\tpower: " << mytv.m_power << std::endl;
            std::cout << "\tname: " << mytv.m_name << std::endl;
            postTvRepresentation(curResource);
        }
        else
        {
...
// Local function to put a different state for this resource
void putTvRepresentation(std::shared_ptr<OCResource> resource)
{
    if(resource)
    {
        OCRepresentation rep;

        std::cout << "Putting Tv representation..."<<std::endl;
...
void onGet(const HeaderOptions& headerOptions, const OCRepresentation& rep, const int eCode)
{
    try
    {
        if(eCode == OC_STACK_OK)
        {
            ...
            putTvRepresentation(curResource);
        }
        else
        {
...
// Local function to get representation of Tv resource
void getTvRepresentation(std::shared_ptr<OCResource> resource)
{
    if(resource)
    {
        std::cout << "Getting Tv Representation..."<<std::endl;
...
void foundResource(std::shared_ptr<OCResource> resource)
{
    ...
    try
    {
		...
		static int startedThread = 0;
			...
			#else
                pthread_create (&threadId, NULL, ChangeTvRepresentation, NULL);
			#endif
                startedThread = 1;
            }
...
int main(int argc, char* argv[]) {
    ...
    try
    {
        ...
        // Find all resources
        requestURI << OC_RSRVD_WELL_KNOWN_URI;// << "?rt=core.Tv";
