...
DeviceProfileTable::DeviceProfileTable(SQLiteDatabase &db)
    : SQLiteTable(db, "device_profiles")
{
    ...
    sql0.append(FIELD_SUPTYPE).append(" TEXT, ");
...
{/*{{{*/
    ...
    values[1] = SQLText(info.nSuperType);
    values[2] = SQLText(info.nDeviceName);
    values[3] = SQLText(info.nDeviceManu);
    values[4] = SQLText(info.nDeviceVer);
    values[5] = SQLText(info.nIconId);
    values[6] = SQLText(info.nJsonData);
    values[7] = SQLText(info.nScriptData);

    std::string sql("REPLACE INTO ");
    sql.append(tableName()).append("(");
    sql.append(FIELD_DEVTYPE).append(", ");
    sql.append(FIELD_SUPTYPE).append(", ");
...
{/*{{{*/
    ...
    if (rs) {
        while (rs->next()) {
            ...
            info.nSuperType = assignSafe(rs->columnText(1));
            info.nDeviceName = assignSafe(rs->columnText(2));
            info.nDeviceManu = assignSafe(rs->columnText(3));
            info.nDeviceVer = assignSafe(rs->columnText(4));
            info.nIconId = assignSafe(rs->columnText(5));
            info.nJsonData = std::move(assignSafe(rs->columnText(6)));
            info.nScriptData = std::move(assignSafe(rs->columnText(7)));
