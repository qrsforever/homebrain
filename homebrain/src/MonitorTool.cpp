...
#ifdef HAVE_CLOUD_MANAGER
#include "HBCloudManager.h"
extern "C" HB::HBCloudManager& cloudManager();
#endif
...
{/*{{{*/
    _Insert("getBuildInfo",         &MonitorTool::_GetBuildInfo);
    ...
    _Insert("makeInstance",         &MonitorTool::_MakeInstance);
    _Insert("deleteInstance",       &MonitorTool::_DeleteInstance);
    ...
    _Insert("setVirtualMode",       &MonitorTool::_SetVirtualMode);

}/*}}}*/

std::string MonitorTool::_GetBuildInfo(const char *params)
{/*{{{*/
    ...
    if (ps[0] == "clips") {
        ...
        std::string user;
        std::string date;
        ruleEngine().getEnvBuildInfo(ver, user, date);
        res.append(ver).append(TOCKEN).append(user).append(TOCKEN).append(date).append(TOCKEN).append("Release");
    } else if (ps[0] == "rule-engine") {
#ifdef HAVE_BUILD_INFO
        res.append(HB_BUILD_VERSION).append(TOCKEN).append(HB_BUILD_USER).append(TOCKEN);
        res.append(HB_BUILD_DATETIME).append(TOCKEN);
#ifndef DEBUG
        res.append("Release");
#else
        res.append("Debug");
...
std::string MonitorTool::_MakeInstance(const char* params)
{/*{{{*/
    std::vector<std::string> ps = stringSplit(params, TOCKEN);
    if (ps.size() != 2) {
        LOGE("params[%d] format error!\n", ps.size());
    ...
    payload->mClsName = ps[0];
    payload->mInsName = innerOfInsname(ps[1]);
    ruleHandler().sendMessage(ruleHandler().obtainMessage(RET_INSTANCE_ADD, payload));
    return "0";
}/*}}}*/

std::string MonitorTool::_DeleteInstance(const char* params)
{/*{{{*/
    std::vector<std::string> ps = stringSplit(params, TOCKEN);
    if (ps.size() != 1) {
        LOGE("params[%d] format error!\n", ps.size());
    ...
    ruleHandler().sendMessage(ruleHandler().obtainMessage(RET_INSTANCE_DEL, payload));
    return "0";
}/*}}}*/
...
{/*{{{*/
    ...
    return ruleEngine().getRuleScript(ps[0]);
    /*
...
{/*{{{*/
    ...
    else if (ps[0] == "template")
        val = DEBUG_SHOW_TEMPLATES;
...
std::string MonitorTool::_SetVirtualMode(const char *params)
{/*{{{*/
#ifdef HAVE_CLOUD_MANAGER
    std::vector<std::string> ps = stringSplit(params, TOCKEN);
    if (ps.size() != 1) {
    ...
    cloudManager().propertyControl("surper.virtual.device", "mode", ps[0]);
    return "0";
#endif
    return "-1";
}/*}}}*/
