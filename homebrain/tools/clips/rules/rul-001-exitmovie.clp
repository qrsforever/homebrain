(defrule rul-001-exitmovie
    ?sct <- (object (is-a SceneContext)
         (where room1) (target exitmovie) (action ?action)
         (services $?svrs)
    )
  ...
  =>
    (bind ?c (create-rule-context rul-001-exitmovie))
    (logi "scene exitmovie, action: " ?action)
    (if (eq ?action exit)
     then
    ...
    (if (eq ?action enter)
     then
        ...
        (assert (start-service ?sct autolight:2 ?envsonsor ?did2 ?ill ?step ?tct))
        (assert (start-service ?sct autolight:3 ?envsonsor ?did3 ?ill ?step ?tct))
        (return)
    )
)
