...
# }}}
    ...
    def createRuleControlView(self, tab):# {{{
        ...
        self.make_instance_btn = ttk.Button(sel_frm, width=5,
                text=gStrings['makeInstance'][self.lan],
                command=lambda : self.onInstanceAddDelDialog("make"))
        self.make_instance_btn.pack(side=tk.LEFT, padx=(5,0))

        self.delete_instance_btn = ttk.Button(sel_frm, width=5,
                text=gStrings['deleteInstance'][self.lan],
                command=lambda: self.onInstanceAddDelDialog("delete"))
        self.delete_instance_btn.pack(side=tk.LEFT, padx=(5,0))
        if len(inses) == 0:
            self.delete_instance_btn.configure(state=tk.DISABLED)
...
# }}}
    ...
    def createRuleControlView2(self, tab):# {{{
        ...
        self.makeins_uuid = tk.StringVar()
...
# }}}
    ...
    def onDeviceSelected(self, *args):# {{{
        ...
        if len(inses) > 0:
            values = inses.split(TOCKEN)
            self.delete_instance_btn.configure(state=tk.NORMAL)
        else:
            self.delete_instance_btn.configure(state=tk.DISABLED)
...
# }}}
    ...
    def onInstanceAddDelDialog(self, opt):# {{{
        width = 320
        height = 120
        self.instance_dialog = tk.Toplevel()
        self.instance_dialog.title(gStrings['insadddeltitle'][self.lan])
        alignstr = '%dx%d+%d+%d' % (
                width, height,
                (self.screenwidth - width) / 2,
                (self.screenheight - height) / 2)
        self.instance_dialog.geometry(alignstr)
        dialog_frm = ttk.Frame(self.instance_dialog,
                padding=10, width=width, height=height)
        ...
        if opt == 'delete':
            ...
            ttk.Label(dialog_frm, font=('Arial', 14),
                    text= self.ins_list.get() + " ?",
                    ).grid(row=1, column=1, columnspan=2, padx=(0,2))
        else:
            ttk.Label(dialog_frm,
                    text=gStrings['uuid'][self.lan],
                    ).grid(row=1, padx=(5, 2))
            self.makeins_uuid.set('')
            ttk.Entry(dialog_frm, font=('Arial', 14), textvariable=self.makeins_uuid,
                    ).grid(row=1, column=1, columnspan=2, padx=(0,2))

        ttk.Button(dialog_frm,
                text = gStrings['confirm'][self.lan],
                command = lambda : self.onClickConfirm(opt+'instance')
                ).grid(row=3, column=1, padx=12, pady=(20, 20))
        ttk.Button(dialog_frm,
                text = gStrings['cancel'][self.lan],
                command = lambda : self.onClickCancel(opt+'instance')
...
# }}}
    ...
    def onClickConfirm(self, who):# {{{
        ...
        if who == 'makeinstance':
            self.instance_dialog.destroy()
            self.tcp.command('makeInstance', self.device_list.get(), self.makeins_uuid.get())
            time.sleep(1)
            self.onDeviceSelected()
            return

        if who == 'deleteinstance':
            self.instance_dialog.destroy()
            self.tcp.command('deleteInstance', self.ins_list.get())
            time.sleep(1)
            self.onDeviceSelected()
            return
