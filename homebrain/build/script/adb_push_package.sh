...
if [[ x$adb == x ]]
then
    echo "not install adb"
    exit 0
fi

BOX_IP=192.168.124.16

ret=`$adb devices | grep $BOX_IP`
if [[ x$ret == x ]]
then
    $adb connect $BOX_IP
fi
...
$adb connect $BOX_IP
$adb remount
$adb shell rm -r /data/homebrain
$adb push data/homebrain /data/
$adb push system/lib64/*.so /system/lib64/
