...
if [[ -f $current_dir/staging_dir.tar.gz ]]
then
    if [[ ! -d $output_dir/staging_dir ]]
    then
        tar zxvf $current_dir/staging_dir.tar.gz -C $output_dir
    fi
else
    echo "Warning: not found $current_dir/staging_dir.tar.gz!"
    exit -1
fi
